...
const AIAssistant = () => {
  ...
  const handleSaveApiKey = () => {
    if (!apiKeyInput.trim()) {
      toast.error('Entrez une clé valide');
      return;
    }
  ...
  const getFallbackResponse = text => {
    ...
    if (lower.includes('tâche') || lower.includes('task')) return 'Configurez votre token GitHub (bouton 🔑) pour utiliser GitHub Models (gratuit). Sinon, utilisez les actions rapides.';
    if (lower.includes('sprint') || lower.includes('analyse')) return 'Pour une analyse de sprint détaillée, configurez votre token GitHub avec le bouton 🔑.';
    if (lower.includes('story') || lower.includes('user')) return 'Configurez votre token GitHub pour générer des user stories professionnelles.';
    return 'Configurez votre token GitHub (bouton 🔑) pour débloquer toutes mes capacités via GitHub Models !';
  };
  ...
  const handleQuickAction = async actionId => {
    switch (actionId) {
      case 'tasks':
        addMessage('user', 'Génère des tâches pour mon projet');
        setLoading(true);
        await new Promise(r => setTimeout(r, 500));
        const projectNames = projects.map(p => `"${p.name}"`).join(', ');
        addMessage('assistant', `Je peux générer des tâches ! Dites-moi ce que vous voulez réaliser.\n\nProjets disponibles : ${projectNames || 'Aucun projet'}\n\nExemple : *"Génère 5 tâches pour le module d'authentification"*`);
        setLoading(false);
        break;
      case 'story':
        addMessage('user', 'Génère une User Story');
        setLoading(true);
        await new Promise(r => setTimeout(r, 500));
        addMessage('assistant', `Décrivez la fonctionnalité que vous voulez transformer en User Story.\n\nExemple : *"En tant qu'utilisateur, je veux pouvoir réinitialiser mon mot de passe"*`);
        setLoading(false);
        break;
      case 'sprint':
  ...
  const handleSprintAnalysis = async () => {
    if (allSprints.length === 0) {
      ...
      addMessage('assistant', 'Aucun sprint trouvé. Créez d\'abord un sprint dans un projet.');
      return;
    }
